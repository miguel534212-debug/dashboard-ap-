          ...
          <tbody>
            ...
            {invoices.map(inv => {
              ...
              return (
                <tr
                  key={inv.id}
                  className="border-b border-gray-700/30 hover:bg-white/[0.02] transition-colors cursor-pointer"
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center" onClick={e => e.stopPropagation()}>
                    {inv.pdfAttachment ? (
                      <span className="inline-flex items-center justify-center p-1.5 text-gray-500">
                  ...
                  <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      {inv.status !== 'Pagada' && (
                        <>
                          {inv.status !== 'Aprobada' && inv.status !== 'En Disputa' && inv.status !== 'Esperando Soporte' && (
                            <button onClick={() => onMarkStatus(inv.id, 'Aprobada')} title="Aprobar" className="p-1.5 text-gray-500 hover:text-blue-400 transition-colors">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            </button>
                          )}
                          <button onClick={() => onMarkStatus(inv.id, 'Pagada')} title="Marcar pagado" className="p-1.5 text-gray-500 hover:text-emerald-400 transition-colors">
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                          </button>
                        </>
                      )}
                      <button onClick={(e) => { e.stopPropagation(); onEdit(inv); }} title="Editar" className="p-1.5 text-gray-500 hover:text-[#3B82F6] transition-colors">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); onDelete(inv.id); }} title="Eliminar" className="p-1.5 text-gray-500 hover:text-red-400 transition-colors">
